\setcounter{page}{1}
\section*{Zielsetzung}
In dem Versuch V27 soll der \emph{Zeeman-Effekt} experimentell beobachtet werden. %experimentell
Hierzu wird die Aufspaltung der Spektralinien einer Kadmium Spektrallampe %der Spektrallinien, nicht der Energieniveaus
...
\end{align*}
Mit $\map{g_S}$ wird der Landé-Faktor des Elektrons bezeichnet und trägt den Wert zwei trägt. %Landé, , der den Wert 2 trägt
Auffälig ist, dass das magnetische Spinmoment $\vec{\mu}\ua{s}$ des Elektrons exakt doppelt so groß ist. %auffällig, in unserer Betrachtung exakt doppelt so groß
so groß ist wie das magnetische Bahnmoment $\vec{\mu}\ua{l}$. %magnetische
...
die der einzelnen Elektronen untereinander unterschieden werden. Die Wechselwirkungen hängen im Wesentlichen %die statt deren
...
$\vec{\mu}$ auf Grund des Landé-Faktors betragsmäßig größer als $\vec{J}$, daher präzediert das magnetische Moment um den Gesamtdrehimpuls. %Landé
Wird über die Präzessionsbewegung zeitlich gemittelt, so fällt auf, dass die $\vec{\mu}_{\perp}$ verschwindet. %auf, dass
Um den Landé-Faktor, nach Formel %Landé, komma nach Formel zu viel
\begin{equation}
  \label{eq:lande_faktor}
  \map{g_J}=\frac{3J(J+1)+S(S+1)-L(L+1)}{2J(J+1)},  %evtl dafür hier komma
\end{equation}
ohne Angabe eines Beweises, zu berechnen.
Aus der Quantenmechanik folgt weiter, dass nur solche Winkel zwischen $\vec{\mu}$ und %weiter, dass
...
\end{equation*}
mit den Energiewerten $E_\alpha$. Der Index $\alpha$ soll andeuten, dass die Wellenfunktion %ohne bei, andeuten, dass
...
%ich glaube er hatte mir gesagt, dass wir auch einmal die matrixelemente erwähnen sollen %HIer noch ergänzen
...
Für diesen Spezialfall ist der Landé-Faktor $\map{g_J}=1$ für alle Quantenzahlen $L$ und $J$. %Landé
...
Übergänge mit einem $\sigma$ emittieren zirkular polarisiertes Licht, die mit einem %zirkular

\setcounter{page}{1}
\section*{Zielsetzung}
In dem Versuch V27 soll der \emph{Zeeman-Effekt} experiementell beobachtet werden. %experimentell
Hierzu wird die Aufspaltung der Energieniveaus einer Kadmium Spektrallampe %der Spektrallinien, nicht der Energieniveaus
...
\end{align*}
Mit $\map{g_S}$ wird der Lande-Faktor des Elektrons bezeichnet und trägt den Wert $\map{g_S}\approx 2$. %Landé, , der den Wert 2 trägt
Auf fallend ist, dass das magnetische Spinmoment $\vec{\mu}\ua{s}$ des Elektrons in etwa doppelt %auffällig, in unserer Betrachtung exakt doppelt so groß
so groß ist wie das magentische Bahnmoment $\vec{\mu}\ua{l}$. %magnetische

\subsubsection{Wechselwirkung der Magnetischen Momente}

Bei dem Übergang zu Mehrelektronatomen müssen die Wechselwirkungen der Momente und Drehimpulse untereinander und %Mehrelektronenatomen,
deren der einzelnen Elektronen untereinander unterschieden werden. Die Wechselwirkungen hängen im Wesentlichen %die statt deren
...
\end{equation*}
eingeführt. Das verwenden von $\vec{L}$ und $\vec{S}$ ist nicht mehr möglich. %Verwenden
Stattdessen existiert
\begin{equation*}
  \vec{J}=\sum_{i=1}^{Z} \vec{j}_i
\end{equation*}
der Gesamtdrehimpuls der Hülle.
Zwischen der LS-Kopplung und der jj-Kopplung gibt es einen kontinuierlichen Übergang. %vielleicht nochmal bezug zur Kernladungszahl

\subsubsection{Energieaufspaltung}
Unter Berücksichtigung der LS-Kopplung kann ein magnetisches Moment $\vec{\mu}$ für den Gesamtdrehimpuls %definiert/eingeführt werden
\begin{equation*}
  \vec{\mu}=\vec{\mu}\ua{L} + \vec{\mu}\ua{S}.
\end{equation*}
Die Richtung von $\vec{J}$ und $\vec{\mu}$ fallen im Allgemeinen nicht zusammen. Zusätzlich ist
$\vec{\mu}$ auf Grund des Lande-Faktors betragsmäßig größer als $\vec{J}$, daher präzediert das magnetische Moment um den Gesamtdrehimpuls. %Landé
Wird über die Präzessionsbewegung zeitlich gemittelt, so fällt auf das die $\vec{\mu}_{\perp}$ verschwindet. %auf, dass
Um den Lande-Faktor zu berechnen, kann die Formel, %Landé, komma nach Formel zu viel
\begin{equation}
  \label{eq:lande_faktor}
  \map{g_J}=\frac{3J(J+1)+S(S+1)-L(L+1)}{2J(J+1)} %evtl dafür hier komma
\end{equation}
ohne Angabe eines Beweises, verwendet werden.
Aus der Quantenmechanik folgt weiter das nur solche Winkel zwischen $\vec{\mu}$ und %weiter, dass
...
\end{equation*}
mit den Energiewerten $E_\alpha$. Der Index $\alpha$ bei soll andeuten das die Wellenfunktion %ohne bei, andeuten, dass
...
Superpositionieren wir die jeweiligen Wellengleichungen und erhalten %superpositionieren
\begin{equation*}
...
ausgestrahlt. Bei $\Delta m=\pm 1$ wird ziruklarpolarisiertes Licht emittiert, ebenfalls parallel zum Magnetfeld. %welche achse meinst du?
%ich glaube er hatte mir gesagt, dass wir auch einmal die matrixelemente erwähnen sollen
...
Für diesen Spezialfall ist der Lande-Faktor $\map{g_J}=1$ für alle Quantenzahlen $L$ und $J$. %Landé
...
Übergänge mit einem $\sigma$ emittieren zirkulär polarisiertes Licht, die mit einem %zirkular

...
\section{Theorie}
Die Funktionsweise eines Rastertunnelmikroskops (RTM) beruht auf den, aus der Quantenmechanik bekannten, %dem
\emph{Tunneleffekt}. Dieser erlaubt es Elektron in klassisch verbotene Bereiche zu \emph{tunneln}. %Elektronen
...
Das Rastertunnelmikroskop bietet die Möglichkeiten im Bereich von $\SI{1}{\angstrom}$ genau aufzulösen. %denke noch genauer
...
Mit Hilfe der Rastertunnelmikroskopie werden Proben so detailreich aufgelöst werden, dass thermische Effekte eine Rolle spielen. %kein werden nach detailreich
So haben thermische Drifts eine Temperaturänderung, zurückzuführen sind eine signifikante Auswirkung auf die Messung. %satz überarbeiten
So beeinflussen z.\,B. thermische Drifts die Signifikanz einer Messung. %Abschnitt nochmal überarbeiten
...
Damit die Fehler die durch Dirfts verursachten verkleinert werden, %Drifts, Satz überarbeiten
wird eine Messung immer nur in eine Richtung, zum Beispiel von rechts nach links, durchgeführt. %nicht klar warum die dadurch verkleinert werden.. wir unterschieden nur bewusst die Richtungen
...
Stoffe, die dem piezoelektrischen Effekts Effekt unterliegen werden als Piezokristall %doppelt Effekt
...
\begin{figure}[!h]
  ...
  \caption{Schematische Darstellung der Nichtlineraität der Auslenkung eines Piezokristalls \cite{rtm}.} %Ausdehnung
  \label{fig: non_linear}
\end{figure}

Neben der Nichtlineratiät der Ausdehnung verschlechtern noch weitere %linearität
...
das bei langer nicht Verwendung des Kristalls die Ausdehnungsfähigkeit verringert. Erklärbar ist der Effekt mit den %dass, verb fehlt
Dipolmomenten im Kristall, die Momente richten sich bei langer nicht Benutzung willkürlich aus und verringern %wenn dann Nichtbenutzung oder anderes Wort
...
\begin{figure}[!h]
  ...
  \caption{Schmeatische Darstelung des Verhalten eines Piezokristall bei häugiger und weniger Benutzung \cite{rtm}.} %Schematische Darstellung, kristalls, häufiger
  \label{fig: ageing}
\end{figure}
...
Das Phänomen der \emph{Kreuzkopplung} beschreibt die $x$- und $y$-Abhängigkeit der $z$-Komponente des Piezokristall (vgl. Abb. \ref{fig: cross_copeling}). %kristalls
\begin{figure}[!h]
  \centering
...
an Ordnung auszeichnet. Deshalb wird diese Graphitstruktur oftmals verwendet, um sich mit einem Rastertunnelmikroskop vertraut zu machen. %und zur Kallibrierung
Die atomare Struktur von HOPG, welche durch die Van Der Waals Kraft zusammengehalten wird, %nur die schichten, atome untereinander über kovalente Bindung (nicht sicher, nochmal nachlesen)
...
$\SI{1.45}{\angstrom}$. %Vielleicht besser den für uns relevanten ABstand aufführen

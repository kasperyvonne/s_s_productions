...
for i in ['foreward', 'backward']:
    ...
    plt.plot(x_h, y_h, 'rx', label = 'Messwerte')
    plt.plot(x_v, y_v, 'bx', label = 'Messwerte')
    params_h, cov_h = curve_fit(linear, x_h, y_h)
    params_v, cov_v = curve_fit(linear, x_v, y_v)
    plot_x_h = np.linspace(x_h[0], x_h[-1], 100)
    plt.plot(plot_x_h, linear(plot_x_h, *params_h), 'r-', label = 'Regression')
    plot_x_v = np.linspace(x_v[0], x_v[-1], 100)
    plt.plot(plot_x_v, linear(plot_x_v, *params_v), 'b-', label = 'Regression')
    plt.xlim(0, 2500)
    plt.ylim(0, 2500)
    plt.legend()
    plt.imshow(img)
    ...
    params_h = correlated_values(params_h, cov_h)
    params_v = correlated_values(params_v, cov_v)
    ...
    theta = acos((delta_x_v * delta_x_h + delta_y_v * delta_y_h) / (sqrt(delta_x_v**2 + delta_y_v)*sqrt(delta_x_h**2 + delta_y_h)))
    ...
    theta_scaled = acos(np.dot(vec_h, vec_v)/(sqrt(vec_v[0]**2 + vec_v[1]**2) *sqrt((vec_h*vec_h).sum()) )     )

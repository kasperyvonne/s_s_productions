...
\end{equation}
berechnet. Die zugehörige Standardabweichung des Mittelwertes lautet: %Standard
...
Weiterhin werden Fehler von Funktionen $f$ mit der Gaußscheen Fehlerfortpflanzung berechnet: %Gaußschen, zweimal fortpflanzung
...
\subsection{Volumenbestimmung} %bestimmung

Um das Saugvermögen der Drehschieber- bzw. Turbopumpe zu bestimmen, wird das Gesamtvolumen des Rezipienten benötigt. %vielleicht lieber des gesamten Rezipienten
...
Für die Volumenberechnung wird jedes Bauteil als Zylinder genähert. Für Volumen und Fehler werden die folgenden Gleichungen verwendet %genähert
...
Zu Beginn wird das Volumen des Tankes bestimmt, dieser ist in der Abbildung \ref{fig: aufbau} das mit der Nummer
\circled{1} versehen Bauteil. %zweimal in
Der Tank, der aus drei Zylindern zusammengesetzt ist, besitzt die Abmessungen: %Zylindern
...
\end{align*}
Von den angegebenen Radien wird die Dicke des Tankes $d=\SI{3\pm1}{\milli\meter}$ abgezogen. %präsenz, bereits
Für das Volumen des Tankes ergibt sich
\begin{equation}
  \label{eq:volumen Tank}
  V\ua{tank}=\SI{8.4\pm0.6}{\litre}.
\end{equation}
Die Fehler der Einzelvolumen werden mit %präsenz
\begin{equation}
...
Bei dem Drehschieberaufbau kommt noch das Volumen des großen Schlauchs \circled{13} hinzu %Schlauchs
\begin{equation*}
...
\begin{equation}
  ...
  p_{\mathrm{E}}=\SI{10 \pm 2 }{\micro\bar}
\end{equation}
...
Die Mittelwerte und die Fehler der Zeitmessung werden mit den Gleichungen \eqref{eq: mittel} und %den
\eqref{eq: standartabweichung} bestimmt. Außerdem wird um den Fehler von $\ln(\frac{ p(t)-p_{ \mathrm{E} } }{ p_0-p_{ \mathrm{E} } })$
zu bestimmen, die Gaußsche Fehlerfortpflanzung verwendet; %komma

\begin{equation}
  \label{eq: fehler_ln_druck}
  \sigma_{ln(p')}=\sqrt{\frac{\sigma_{p}^{2}}{\left(p - p_{E}\right)^{2}} + \frac{\sigma_{p_{0}}^{2}}{\left(p_{0} - p_{E}\right)^{2}} + \frac{\sigma_{p_{E}}^{2} \left(p_{0} - p_{E}\right)^{2}}{\left(p - p_{E}\right)^{2}} \left(\frac{p - p_{E}}{\left(p_{0} - p_{E}\right)^{2}} - \frac{1}{p_{0} - p_{E}}\right)^{2}}.
\end{equation}
...
Die gemittelten Zeiten werden mit \eqref{eq: mittel} und \eqref{eq: standartabweichung} bestimmt. %präsenz
Eine grafische Visualisierung der Werte ist in den Abbildung \ref{fig: leck_dreh_1} und \ref{fig: leck_dreh_2} dargestellt. %zweimal darstellung
...
Abschließend werden die aus der Druckkurve und Leckratenmessung experimentell bestimmten Saugvermögen in einem Plot (vgl. Abb. \ref{fig: dreh_druck_leck})
aufgetragen.
...
\begin{equation}
  \label{eq:enddruck_drehschieber}
  p_{\mathrm{E}}=\SI{9.0 \pm 0.9}{\nano\bar}. %würde vielleicht allgemein eher millibar verwenden
\end{equation}

\subsubsection{Auswertung der Druckkurve}
Die aufgenommenen Messwerte für die Druckkurve sind in Tabelle \ref{tab: druck_turbo}
aufgeführt. Die Fehler der logarithmierten Werte werden mit Gleichung \eqref{eq: fehler_ln_druck} errechnet. %logar...
\input{../Messdaten/table/turbo/druck_messdaten_final.tex}

Die grafische Darstellung der Messwerte ist in Abbildung \ref{fig: druck_turbo} zu finden. %zu finden
...
\end{figure}
\FloatBarrier
Darüber hinaus wird in der Abbildung \ref{fig: druck_turbo_zoom} ein Teil der Abbildung \ref{fig: druck_turbo} vergrößert dargestellt. %darüber hinaus, zweimal wird
...
\end{aligned}
\end{align}
Mit Hilfe der Gleichung \eqref{eq: saug_druck} ist eine Saugvermögenbestimmung möglich. %keine kraft
...
Die aufgenommenen Messwerte für die Leckratenmessung der Turbopumpe sind in den Tabellen \ref{tab: leck_turbo_leck_0.1.pdf} - \ref{tab: leck_turbo_leck_0.4.pdf} %aufgenommenen
abgebildet. Der Gleichgewichtsdruck wird hierbei mit $p\ua{g}$ bezeichnet. %hierbei

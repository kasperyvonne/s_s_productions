...
\end{equation}
berechnet. Die zugehörige Standartabweichung des Mittelwertes lautet: %Standard
...
Weiterhin werden die Fehler mit der Gaußsche Fehlerfortpflanzung fortgepflanzt: %Gaußschen, zweimal fortpflanzung
...
\subsection{Volumenbestimung} %bestimmung

Um das Saugvermögen der Drehschieber- bzw. Turbopumpe zu bestimmen, wird das Gesamtvolumen des verwendeten Versuchsaufbaus benötigt. %vielleicht lieber des gesamten Rezipienten
...
Für die Volumenberechnung wird jedes Bauteil als Zylinder genährt. Für Volumen und Fehler werden die folgenden Gleichungen verwendet %genähert
...
Der Tank, der aus drei Zylinder zusammengesetzt ist, besitzt die Abmessungen: %Zylindern
...
\end{align*}
Von den angegebenen Radien wurde bereites die Dicke des Tankes $d=\SI{3\pm1}{\milli\meter}$ abgezogen. %präsenz, bereits
Für das Volumen des Tankes ergibt sich
\begin{equation}
  \label{eq:volumen Tank}
  V\ua{tank}=\SI{8.4\pm0.6}{\litre}.
\end{equation}
Die Fehler der Einzelvolumen wurden mit %präsenz
\begin{equation}
...
Bei dem Drehschieberaufbau kommt noch das Volumen des großen Schlauches \circled{13} hinzu %Schlauchs
\begin{equation*}
...
Die Mittelwerte und die Fehler der Zeitmessung werden mit Gleichungen \eqref{eq: mittel} und %den
...
Die gemittelten Zeiten wurden mit \eqref{eq: mittel} und \eqref{eq: standartabweichung} bestimmt. %präsenz
Eine grafische Darstellung der Werte ist in den Abbildung \ref{fig: leck_dreh_1} und \ref{fig: leck_dreh_2} dargestellt. %zweimal darstellung
...
Abschließend werden die sich aus der Druckkurve und Leckratenmessung bestimmen %satz
...
aufgeführt. Die Fehler der lograrithmierten Werte werden mit Gleichung \eqref{eq: fehler_ln_druck} errechnet. %logar...
\input{../Messdaten/table/turbo/druck_messdaten_final.tex}

Die grafische Darstellung der Messwerte ist in Abbildung \ref{fig: druck_turbo} zufinden. %zu finden
...
\end{figure}
\FloatBarrier
Darüberinaus wird in der Abbildung \ref{fig: druck_turbo_zoom} wird ein Teil der Abbildung \ref{fig: druck_turbo} vergrößert dargestellt. %darüber hinaus, zweimal wird
...
\end{aligned}
\end{align}
Mit Hilfe der Gleichung \eqref{eq: saug_druck} ist eine Saugkraftbestimmung möglich. %keine kraft
...
Die aufgenommen Messwerte für die Leckratenmessung der Turbopumpe sind in den Tabellen \ref{tab: leck_turbo_leck_0.1.pdf} - \ref{tab: leck_turbo_leck_0.4.pdf} %aufgenommenen
abgebildet. Der Gleichgewichtsdruck wird hierbeich mit $p\ua{g}$ bezeichnet. %hierbei

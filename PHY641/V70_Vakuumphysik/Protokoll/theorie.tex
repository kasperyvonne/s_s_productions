...
\end{equation}
Hier entspricht $n$ der Stoffmenge und $D$ dem Moleküldurchmesser. Mit fallendem Druck wächst die mittlere freie  %Zwei mal mit
...
\begin{equation}
  S = \frac{\dif{}}{\dif{t}} V.
\end{equation}
Das Saugvermögen wird oft bezogen auf das Einheitsvolumen in $[{S}] = \si{\per \second}$ angegeben. %Würde das mit dem Einheitsvolumen weglassen, da es im folgenden nicht mehr verwednet wird
...
Der Rotor einer Turbomolekularpumpe ist in Abbildung~\ref{fig: turbo} zu sehen. Dieser besteht im Wesentlichen aus einer %\ref{}
...
nach einer gewissen Verweildauer mit einer durch den Rotor erzeugten Geschwindigkeitkomponente nach schräg unten. Anschließend
wird es von einem Stator aufgefangen und gelangt somit in einen weiter unten gelegenen Rotoreinfangbereich.
Hierdurh entsteht eine Molekularströmung, mit der ein Vakuum erzeugt werden kann. Voraussetzung für den beschriebenen % Hier und danach Hierdurch als Satzanfang klingt nicht so schön
...
\begin{subfigure}{0.49\textwidth}
  ...
  \label{fig: turbo}
\end{subfigure}
\caption{Grafiken zur Erläuterung der Funktionsweise einer Drehschieber- und einer Turbomolekularpumpe.}  %Drehschieber-
...
eine spiralförmige Anode, in derem Zentrum ein Glühdraht positioniert ist. Der Einsatz des Ionisationsvakuummeters ist erst ab einem %Schreib anstatt von Gerät leiber I  onisationsmannometer
...
$W = I^2 R$ und die durch Wärmeleitung an das Gas abgebene Leistung bestimmen die Temperatur des Drahtes. Die benötigte % Satz nicht wieder mit Die anfangen

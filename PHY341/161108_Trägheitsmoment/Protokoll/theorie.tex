...
lässt sich mit Hilfe der Dichte des Körpers $\rho(\vec{r}) = dm/dV$ umformulieren zu:
\begin{equation}
  I = \int \rho(\vec{r})\cdot r^2 dV
\end{equation}
...
\begin{equation}
  D_{r} = - D \cdot \phi
\end{equation}
Mit \refeq{eq: drehmoment} ergibt sich die homogene Differentialgleichung:
\begin{equation}

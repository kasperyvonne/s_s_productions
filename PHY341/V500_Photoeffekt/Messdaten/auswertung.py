...
def auswertung(spannungen, stromstaerke,name, plus_x, minus_x, plus_y):
	...
	plt.grid()
	plt.legend(loc='best')
	plt.xlabel(r'$\mathrm{Bremsspannung} \, \mathrm{in} \, \mathrm{V}$',size=25)
	plt.ylabel(r'$\sqrt{I_{\mathrm{p}}} \, \, \mathrm{in} \, \, \sqrt{\mathrm{nA}}$',size=25)
	#plt.show()
...
plt.ylim(0,2)
yerr=np.zeros(len(spannungen))
#plt.plot(frequenz*1e-12,spannungen,'rx', label=r'$\mathrm{Bestimmte} \, \mathrm{Gegenspannung}}$')
plt.errorbar( frequenz*1e-12, spannungen + yerr,  yerr=spannungen_fehler, fmt='rx')

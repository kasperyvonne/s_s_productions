...
\begin{equation}
  ...
  \label{eq: ladung}
\end{equation}
Für den durch diese Potentialdifferenz hervorgerufenen Strom $I$ an einen in Reihe geschalteten Widerstand $R$ folgt nach dem ohmschen Gesetz %durch durch, klingt blöd
\begin{equation}
  I = -\frac{U\ua{C}}{R}.
  \label{eq: strom}
\end{equation}
...
\end{equation}
Der Wert $RC$ wird als charakteristische Zeitkonstante des Relaxationsprozesses bezeichnet. Innerhalb dieser Zeitspanne ändert %-wird
...
im Versuch überprüft werden. Um die Ergebnisse in einer Polarebene einzeichnen zu können, werden die Formeln \eqref{eq: phase_theorie} und %ich würde nicht schreiben, um später in eine Polarebene einzeichen zukönne,den Fit könnte ich ja auch fr einen ganzen normalen Graphen nutzen
...
\begin{equation}
  U(t) = RC\,\frac{\dif{U\ua{C}(t)}}{\dif{t}} + U\ua{C}(t). %kleines c
\end{equation}
...
sich ergibt:
\begin{equation}
  U(t) = RC\,\frac{\dif{U\ua{C}(t)}}{\dif{t}} \quad \Leftrightarrow \quad U\ua{C}(t) = \frac{1}{RC}\int_0^t U(\bar{t})\dif{\bar{t}}. %kleines c, nicht \bar sondern \overline
\end{equation}

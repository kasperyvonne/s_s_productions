...
Durch Benutzung der Formel %ausdruck
...
Die gemessenen Werte sind in Tabelle \ref{tab:messwerte_fallzeit_kugel_klein} gelistet. %auf
\begin{table}
\centering
\begin{tabular} {c}
	\toprule
...
dasselbe, wie im Unterkapitel \ref{sec:visko}. %Satz gefällt mir noch nicht... vielleicht: Mit der gefundenen Konstante K und der temperaturabhängigen Dichte(ref) kann nun die dynamische Viskosität berechnet werden.
...
der Wert $\SI{983.20}{\kilogram\per\cubic\meter}$ genährt. %verwendet

Für die Fallzeit der Kugel ergaben sich die in  %würde tempus einheitlich machen
...
\bottomrule
\end{tabular}
\caption{Fallzeiten der großen Kugel im Viskosimeter bei unterschiedlichen Temperaturen} %ausdruck
\label{tab:messwerte_fallzeit_kugel_gross_temo}
\end{table}

Das Ergebnis der Viskositätsberechnung ist in Tabelle \ref{tab:visko_wasser_temp} zufinden. % zu finden
...
\bottomrule
\end{tabular}
\caption{Viskoität von Wasser bei unterschiedlichen Temperaturen} %ausdruck
\label{tab:visko_wasser_temp}
\end{table}

Die Ergebnise werden in Abbildung \ref{fig:t_v_v} und \ref{fig:t_v_l_v} dargestellt und mit den Literaturwerten verglichen.  %Ergebnisse, verglichen werden sie doch erst in der diskussion
...
\caption{Temperatur gegen Viskosität} %umgangssprachlich
\label{fig:t_v_v}
\end{figure}
...
Regression an eine Funktion der Form $T_i = Ae^{\frac{B}{T}}$ bestimmt. %hinter formel ref

Als Parameter ergaben sich:
\begin{equation*}
A=\SI{1.74e-3}{\pascal\second} \quad B=\SI{6.4e2}{\kelvin} %Einheiten ? haben die ne dimension?
\end{equation*}

\subsection{Bestimmung der Reynolds-Zahl}

Die \emph{Reynolds-Zahl} wird durch den in Gleichung \eqref{eq: reynolds} %verb

Dabei sei $d$ der Rohrdurchmesser und $v$ die Fallgeschwindigkeit der Kugel.
Der Rohrdurchmesser wird durch den Durchmesser der großen Kugel angenährt. %genähert
...
Die sich damit ergebenen Werte für die \emph{Reynolds-Zahl} sind in Tabelle \ref{tab:rey_visko} %'sich damit ergebenen Werte' weglassen
zu finden.
...
\bottomrule
\end{tabular}
\caption{\emph{Reynolds-Zahl} des Viskosimeter bei unterschiedlichen Temperaturen} %viskosimeters
\label{tab:rey_visko}
\end{table}
Die kritische \emph{Reynolds-Zahl} für Wasser liegt bei $2040$\cite{rey}. %auf theorie verweisen
Deshalb handelt es sich bei der Strömung im Viskosimeter um eine laminare.%ausdruck
 %Literatur

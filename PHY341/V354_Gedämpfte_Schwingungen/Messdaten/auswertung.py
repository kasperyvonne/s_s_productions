...
amplitude_offset = amplitude - amplitude[-1]
print(amplitude_offset)
params_raw, cov = curve_fit(lin_function, time,  np.log(amplitude/amplitude[0]) )

R_eff_korrektur = params_raw[0] * 2 * L
params2_raw, cov2 = curve_fit(exp_function, time, amplitude)
params2 = unp.uarray(params2_raw, np.sqrt(np.diag(cov2)))
print('Parameter der linearen Regression: ', params2)
print('A_0 = ', params2[0])
mu = -1/(2*np.pi) * params2[0]
#print(mu)

print(R_eff_korrektur)
...
plt.clf()
f = np.linspace(9, 46000, 1000)
u = np.linspace(1,4, 100)
plt.plot(frequenz/1000, U_C/U_G, 'rx', label='Messwerte')
#plt.plot(f/1000, (np.ones(len(f))* 1/np.sqrt(2) * 17/4.32), '--b', label = 'Niveaulinie $\\frac{1}{\sqrt{2}}\\frac{U_{C, max}}{U}$')
#plt.plot(np.ones(len(u))* nu_minus_exp/1000, u, '--g', label = 'Abgelesene Frequenzen $\\nu_{\pm}$')
#plt.plot(np.ones(len(u))* nu_plus_exp/1000, u, '--g')
#plt.xlim (24.5, 45.5)
plt.xlim(1*10**(-2.1) ,1*10**(2))
plt.ylim(0.8, 4)
plt.legend(loc='best')
plt.xlabel('Frequenz $\\nu$ in kHz')
plt.ylabel('$\\frac{U_C}{U}$')
...
plt.ylim(-0.2, np.pi)
plt.xscale('log')
plt.savefig('phase_f_log.pdf')


t = np.linspace(time[0]-0.00001, time[-1]+0.00001)
plt.clf()
plt.plot(t* 100000, lin_function(t, *params_raw) , '-b', label='Fit-Funktion')
plt.plot(time*100000, np.log(amplitude/amplitude[0]), 'rx', label = 'Messwerte')
plt.xlim(-1, 37)
plt.ylim()
plt.grid()
plt.legend(loc='best')
plt.xlabel('Zeit t in $10^{-5}$ s')
plt.ylabel('$log\left(\\frac{A(t)}{A_0}\\right)$')

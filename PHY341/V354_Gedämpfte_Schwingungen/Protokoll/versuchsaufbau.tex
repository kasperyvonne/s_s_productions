...
Am Anfang muss die Frequenz des Generator %Generators
so eingestellt werden, dass die Kondensatorspannung um den Faktor $3$ bis $8$
abgenommen hat. Mithilfe der 'Cursor'-Funktion des Oszilloskops, wird dann %Mit Hilfe
die Höhe von allen oberen Schwingungsbäuchen gemessen.

\subsection{Widerstandsbestimmung für den ap. Grenzfall}

Für die Bestimmung des Wiederstandes an dem sich %stil
der aperiodische Grenzfall befindet, wird das Potentiometer
auf seinen Maximalwert eingestellt.
Der Widerstand wird nun kontinuierlich verringt, dabei wird das %verringert
Ozsilloskop beobachtet. Man dreht den Widerstand soweit runter, bis %so weit
...
An dieser Stelle befindest sich der ap. Grenzfall und somit $R\ua{ap}$. %befindet, aperiodische
...
varriiert. Mithilfe der 'Measure'-Funktion des Ozsillsokops %Mit Hilfe, Oszilloskops
kann Generator- und Kondensatorspannung bei verschieden Frequenzen %verschiedenen
abgelesen werden. Zusätzlich wird mit ihr die Phasenlänge $b$ der Generatorspannung
gemessen. Abschließend bietet die 'Cursor'-Funktion eine Möglichkeit, um %ohne um

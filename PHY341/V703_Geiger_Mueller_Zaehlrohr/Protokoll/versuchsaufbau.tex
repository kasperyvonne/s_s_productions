...
Zusehen ist dort das Zählrohr und die nachgeschaltete Elektronik. %und die nachgeschaltete Elektronik klingt blöd
...
\subsubsection{Aufnahme der Charakteristika des Zählrohres} %s.o.
...
\subsubsection{Ostillographische Messung Tot- und Erholungszeit} %Oszillographische
...
Unterschied zwischen der wahren und der gemessenen Zählrate. %wahr unphysikalisch
Der Zusammenhang zwischen der gemessenen Zählrate $N\ua{r}$ und
wahren $N\ua{w}$ lautet: %s.o.
\begin{equation}
  \label{eq:wahre_zaehlrate}
  N\ua{w}=\frac{N\ua{r}}{1-TN\ua{r}}.
\end{equation}
Die Verwendung zweier unterschiedlichen Präparate bietet eine Möglichkeit zu %zweier unterschiedlicher; zur
Bestimmung der Totzeit. Dazu wird bei $\SI{480}{\volt}$ und in eiem Zeitraum $\SI{60}{\second}$ eine neue %einem Zeitraum von
$\ce{^{204}Tl}$ und einer alte $\ce{^{204}Tl}$ Probe untersucht. %kurz erwähnen warum
...
\begin{equation}
  ...
  N_{1+2}<N_1+N_2,
\end{equation}
mit diesem kann dann die Totzeit zu %gemäß
\begin{equation}
  \label{eq:totzeit}
...
\subsubsection{Messung der pro Teilchen vom Zählrohr feigesetzte Ladunfsmenge} %freigesetzten Ladungsmenge

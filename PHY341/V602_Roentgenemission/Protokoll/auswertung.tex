...
\end{figure}
In der Abbildung sind die $K_\alpha$ und $K_\beta$ Kanten mit eingezeichnet. %ref
...
Aus den Formeln \eqref{eq: sigma_1} und \eqref{eq: sigma_2} errechnet sich: %errechnen
\begin{equation}
   \label{eq:abschirm}
...
Die im $2:1$ Kopplungsmodus betriebene Versuchsapperatur wurde wie folgt eingestellt: %präsenz
\begin{equation*}
  \theta\in\left[\theta\ua{K}-2,\theta\ua{K}+2\right]\,\si{\degree},\quad \Delta\theta=\SI{0.1}{\degree}, \quad \Delta t=\SI{20}{\second}. %lit
\end{equation*}
Bei dem Winkel $\theta\ua{K}$ erwartet man für die jeweilige Probe das Impulsmaximum.
Die Liste mit allen zu untersuchenden Materialen und den zugehörogen Winkel ist in Tabelle %Materialien, zugehörigen, Winkeln
...
Die jeweils gemessenen Spektren sind in Abbildung \ref{fig: spektrum_brom_germanium} und %in den Abbildungen
\ref{fig: spektrum_strontium_zirconium} dargestellt. Die zugehörogen Messwerte befinden sich in den Tabellen \ref{tab: zr}- \ref{tab: strom}. %zugehörigen
\begin{figure}
  \centering
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \caption{Gemessenes Spektrum für Brom $\ce{Ge}$. Die zugehörigen Messwerte sind in Tabelle \ref{tab: germanium} zu finden.} %Germanium
...
\end{figure}
Analog zu Kapitel \ref{sec: zink} werden die Enegerien $E\ua{K}$ und Abschirmkonstanten $\sigma\ua{K}$ %Energien
bestimmt. Die Ergebnisse werden in Tabelle \ref{tab: messergebnisse} aufgeführt. %präsenz
\begin{table}
  \centering
...
konfiguriert:
\begin{equation*}
  \theta\in\left[\theta\ua{{L_3}}-2,\theta\ua{{L_2}}+2\right]\,\si{\degree},\quad \Delta\theta=\SI{0.1}{\degree}, \quad \Delta t=\SI{20}{\second}. %L vertauscht
\end{equation*}
Dabei wurden die Winkel %präsenz
\begin{equation*}
  \ua{\theta\ua{L_2}}=\SI{12.95}{\degree} \qquad  \ua{\theta\ua{L_3}}=\SI{14.95}{\degree}
\end{equation*}
aus der Literatur \cite{l_kante} notiert. %wieso notiert?
...
Für die Bestimmung Rydbergenerigie $R_\infty$ wird durch die zuvor %der, energie
...
gerade, %dass ..., Satz besser umformulierenn
\begin{equation*}
  R_\infty=m^2
\end{equation*}
ist. %s.o.
Die in den vorherigen Kapiteln bestimmten Energien sind in Tabelle \ref{tab: ener_ryd} gelistet %auf
und in der Abbildung \ref{fig: ryd_ener} gezeichnet. In der Abbildung ist %ein
...
\begin{figure}
  ...
  \caption{Auftagung der experimentell berechneten Energien $E\ua{K}$ gegen die Kernladungszahl $Z$. Zusätzlich ist in der Abbildung die zugehöroge Ausgleichsgerade zu sehen.} %tragung
  \label{fig: ryd_ener}
\end{figure}

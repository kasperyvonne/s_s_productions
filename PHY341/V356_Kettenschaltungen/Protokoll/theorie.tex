...
\subsection{Bestimmung der Schwingungsgleichungen für eine Kette} %würde nicht $LC$
Die Schwingungsgleichung wird mit den Kirchhoffschen Regelen aufgestellt. %plural
...
\begin{figure}
  ...
  \caption{Kettenglied $n$.\cite{manual}} %Zitat
...
Da \eqref{eq:kreisfrequenz_lc_glied} nur für endlich viele Kettenglieder definertist,
...
\begin{figure}
  ...
  \caption{$C_1C_2$ Kette.\cite{manual}}
...
\end{equation}
Aufgrund des Unterschiedes der beiden Maxima, gibt es einen Frequenzbereich, in dem %aufgrund des Unterschiedes
die Kettenschaltung undruchlässig wird. Dieser ist auch in Abbildung \ref{fig: dispersion_LC1C2} %-auch
zusehen.

\subsection{Ausbreitungsgeschwindigkeit von Wellen in einer Kettenschaltung}
Bei eine Welle der Form \eqref{eq:loesung_gleichung_lc} %bei einer Welle der Form
...
\begin{figure}
  ...
  \caption{Spannungen un Ströme in der $LC$-Kette.\cite{manual}}
  \label{fig:bestimmung_impe}
\end{figure}
\begin{equation*}
I_0 - \map{i}\omega\frac{C}{2}U_0+\frac{U_1-U_0}{\map{i}\omega L}=0 \quad \in \mathbb{C}
\end{equation*}
bestimmt.
...
Obwohl ist $Z$ frequenzabhängig, kann für Frequenzen weit unterhalb der Grenzfrequenz
$Z\approx \sqrt{\frac{L}{C}}$ angenommen werden. %überarbeite diesen satz nochmal
...
zu Überlagerung wie z.\,B. stehende Wellen. %überarbeite diesen satz nochmal
Für die Spannungen bzw. Stromstärken am Kettenende (vgl. Bild \ref{fig:kettenende}) %gilt entweder beides plural oder beides singular

\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{bilder/wellenwiderstand.png}
  \caption{Kettenende.\cite{manual}}
